...
/*   Updated: 2023/01/29 23:46:25 by lidanzhang       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

static void	render_vertical(t_mlx *mlx, t_mlx *temp)
{
	mlx->x0 = mlx->x;
	mlx->y0 = mlx->y;
...
static void	render_horizontal(t_mlx *mlx, t_mlx *temp)
{
	mlx->x0 = mlx->x;
	mlx->y0 = mlx->y;
...
static void	render_vertical_horizontal(t_mlx *mlx, t_mlx *temp)
{
	if (mlx->camera == 0 || mlx->camera == 1)
	{
		if (mlx->y < temp->map_height)
			render_vertical(mlx, temp);
		render_horizontal(mlx, temp);
	}
}
...
void	ft_render(t_mlx *mlx)
{
	...
	while (mlx->y <= temp.map_height)
	{
		...
		{
			render_vertical_horizontal(mlx, &temp);
			mlx->x++;
		}
		mlx->x = 0;
		mlx->y++;
